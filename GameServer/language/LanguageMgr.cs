...
namespace DOL.Language
{
	...
	public class LanguageMgr
	{
        ...
        #region Temporary languageMgr master level arrays
        ...
        {
            return GetAllowedLangKeys().Contains(langKey.ToLower()) ? true : false;
        }

        /// <summary>
        /// Returns an generic list with the language keys of all languages that are allowed to use on the server.
        /// The strings inside this list are already have been lowered (ToLower()).
        /// </summary>
        /// <returns></returns>
        public static List<string> GetAllowedLangKeys()
        {
            List<string> allowedKeys = new List<string>();
            var dbo = GameServer.Database.SelectObject<ServerProperty>("Key='allowed_languages'");

            string[] tmp = dbo.Value.Split(';');
            foreach (string allowedKey in tmp)
                allowedKeys.Add(allowedKey.ToLower());
        ...
        {
            //First, check if the clients language key is an allowed language key. There is no need to check
            //for an english language, because the translation id already is the english translstion
            if (/*language.ToLower() == "en" || */!GetAllowedLangKeys().Contains(language.ToLower()))
            ...
            switch (eKey)
            {
                case eTranslationKey.Ability_Description:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageAbility>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.Ability_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageAbility>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.Area_Description:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageArea>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Description.Trim().Length > 0)
                ...
                case eTranslationKey.GameObject_ExamineArticle:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageGameObject>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Name.Trim().Length > 0)
                ...
                case eTranslationKey.GameObject_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageGameObject>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.Item_ExamineArticle:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageItem>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                ...
                case eTranslationKey.Item_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageItem>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.NPC_ExamineArticle:
                    {
                        if (unique != null && unique.Length > 0)
                        {
                            var dbo = GameServer.Database.SelectObject<DBLanguageNPC>("TranslationId='" + translationID + "' AND TranslationUnique='" + unique + "' AND Language='" + language + "'");
                            if (dbo != null)
                            {
                                //Lets check if we have text
                                if (dbo.ExamineArticle.Trim().Length > 0)
                ...
                case eTranslationKey.NPC_GuildName:
                    {
                        if (unique != null && unique.Length > 0)
                        {
                            ...
                            {
                                //Lets check if we have text
                                if (dbo.GuildName.Trim().Length > 0)
                                    translation = dbo.GuildName;
                                else
                                    translation = translationID;
                            }
                            else
                ...
                case eTranslationKey.NPC_MessageArticleFemale:
                    {
                        if (unique != null && unique.Length > 0)
                        {
                            var dbo = GameServer.Database.SelectObject<DBLanguageNPC>("TranslationId='" + translationID + "' AND TranslationUnique='" + unique + "' AND Language='" + language + "'");
                ...
                case eTranslationKey.NPC_MessageArticleMale:
                    {
                        if (unique != null && unique.Length > 0)
                        {
                            var dbo = GameServer.Database.SelectObject<DBLanguageNPC>("TranslationId='" + translationID + "' AND TranslationUnique='" + unique + "' AND Language='" + language + "'");
                ...
                case eTranslationKey.QuestText:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageQuest>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                ...
                case eTranslationKey.Spell_Description:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageSpell>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Description.Trim().Length > 0)
                ...
                case eTranslationKey.Spell_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageSpell>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.SpellLine_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageSpellLine>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Name.Trim().Length > 0)
                ...
                case eTranslationKey.Style_Description:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageStyle>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Description.Trim().Length > 0)
                ...
                case eTranslationKey.Style_Name:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageStyle>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                ...
                case eTranslationKey.SystemText:
                    {
                        if (unique != null && unique.Length > 0)
                        {
                            var dbo = GameServer.Database.SelectObject<DBLanguageSystem>("TranslationId='" + translationID + "' AND TranslationUnique='" + unique + "' AND Language='" + language + "'");
                            if (dbo != null)
                            {
                                //Lets check if we have text
                                if (dbo.Text.Trim().Length > 0)
                        ...
                        {
                            var dbo = GameServer.Database.SelectObject<DBLanguageSystem>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                            if (dbo != null)
                            {
                                //Lets check if we have text
                                if (dbo.Text.Trim().Length > 0)
                ...
                case eTranslationKey.Zone_Description:
                    {
                        var dbo = GameServer.Database.SelectObject<DBLanguageZone>("TranslationId='" + translationID + "' AND Language='" + language + "'");
                        if (dbo != null)
                        {
                            //Lets check if we have text
                            if (dbo.Description.Trim().Length > 0)
