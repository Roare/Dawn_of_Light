	...
	public abstract class AbstractArea : IArea
	{
		...
		public virtual void OnPlayerLeave(GamePlayer player)
		{
			if (m_displayMessage && Description != null && Description != "")
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, eTranslationKey.SystemText, "(Region) You have left {sourcearea}.",
                    "").Replace("{sourcearea}", LanguageMgr.GetTranslation(player.Client, eTranslationKey.Area_Description, Description, "")),
		...
		public virtual void OnPlayerEnter(GamePlayer player)
		{
			if (m_displayMessage && Description != null && Description != "")
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, eTranslationKey.SystemText, "(Region) You have entered {sourcearea}.",
                    "").Replace("{sourcearea}", LanguageMgr.GetTranslation(player.Client, eTranslationKey.Area_Description, Description, "")),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
